#!/bin/bash
# Deploy FKS Platform to Kubernetes with fkstrading.xyz domain
# This script deploys the platform with domain configuration for Tailscale IP
...
install_nginx_ingress() {
    ...
    helm upgrade --install ingress-nginx ingress-nginx/ingress-nginx \
        ...
        --set controller.service.externalIPs[0]=${TAILSCALE_IP} \
        --wait \
        --timeout 5m
...
# Configure minikube tunnel (if using minikube)
setup_minikube_tunnel() {
    log_info "Setting up minikube tunnel for Tailscale IP..."
    
    if command -v minikube &> /dev/null; then
        if minikube status &> /dev/null; then
            log_info "Minikube detected. Tunnel will be required."
            log_warning "You'll need to run 'minikube tunnel' in a separate terminal to expose services to Tailscale IP"
            log_info "Run this command in another terminal:"
            echo ""
...
test_services() {
    ...
    if curl -s -k "http://${DOMAIN}/health" > /dev/null 2>&1; then
    ...
    if curl -s -k "http://app.${DOMAIN}/health" > /dev/null 2>&1; then
    ...
    sleep 2
    if curl -s -k "http://app.${DOMAIN}/api/v1/signals/latest/BTCUSDT?category=swing&use_ai=false" | grep -q "signal_type" 2>/dev/null; then
        log_success "Bitcoin signal generation working ✓"
    else
        log_warning "Bitcoin signal generation may not be working yet"
    fi
}
...
show_access_info() {
    ...
    if command -v minikube &> /dev/null; then
        if minikube status &> /dev/null; then
            echo "⚠️  IMPORTANT: Run 'minikube tunnel' in a separate terminal to expose services to Tailscale IP"
            echo ""
        fi
    fi
}
