#!/bin/bash
# Review and Fix Kubernetes Environment
# This script reviews the K8s environment and ensures all 14 services are running healthily
...
# All 14 services from service registry
ALL_SERVICES=(
    "fks_web:8000"
    "fks_api:8001"
    "fks_app:8002"
    ...
    "fks_portfolio:8012"
    "fks_monitor:8013"
    # "fks_ninja:8005"  # C# service, not containerized
)
...
review_current_deployment() {
    log_info "Reviewing current deployment..."
    ...
    if helm list -n "$NAMESPACE" | grep -q "$RELEASE_NAME"; then
        log_success "Helm release $RELEASE_NAME exists ✓"
    ...
    kubectl get svc -n "$NAMESPACE" -o wide 2>/dev/null || log_warning "No services found in namespace $NAMESPACE"
    echo ""
    
    # Check deployments
    log_info "Deployment Status:"
    kubectl get deployments -n "$NAMESPACE" -o wide 2>/dev/null || log_warning "No deployments found in namespace $NAMESPACE"
    echo ""
    
    # Check ingress
    log_info "Ingress Status:"
    kubectl get ingress -n "$NAMESPACE" -o wide 2>/dev/null || log_warning "No ingress found in namespace $NAMESPACE"
    echo ""
}
...
deploy_all_services() {
    ...
    # Deploy using Helm with all available services enabled
...
show_comprehensive_summary() {
    echo ""
    echo "╔═══════════════════════════════════════════════╗"
    echo "║  FKS Platform - Kubernetes Environment Review ║"
    echo "╚═══════════════════════════════════════════════╝"
    echo ""
...
main() {
    ...
    echo ""
    log_success "✓ Kubernetes environment review complete!"
    echo ""
}
